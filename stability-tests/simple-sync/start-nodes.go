...
func startNodes() (teardown func(), err error) {
	...
	syncerDataDir, err := common.TempDir("kobrad-datadir-syncer")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCER datadir: %s", syncerDataDir)

	syncedDataDir, err := common.TempDir("kobrad-datadir-synced")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCED datadir: %s", syncedDataDir)

	syncerCmd, err := common.StartCmd("kobrad-SYNCER",
		"kobrad",
	...
	syncedCmd, err := common.StartCmd("kobrad-SYNCED",
		"kobrad",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
