...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("kobrad-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("kobrad datadir: %s", dataDir)

	kobradCmd, err := common.StartCmd("kobrad",
		"kobrad",
	...
	spawn("startNode-kobradCmd.Wait", func() {
		err := kobradCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("kobradCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes kobrad closes ungracefully
				log.Errorf("kobradCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(kobradCmd, "kobradCmd")
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	kobraMinerCmd, err := common.StartCmd("MINER",
		"kobraminer",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
		"-s", rpcAddress,
	...
	spawn("kobra-miner-Cmd.Wait", func() {
		err := kobraMinerCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(kobraMinerCmd, "kobraMinerCmd")
	return nil
}
