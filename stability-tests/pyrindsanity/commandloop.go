...
func commandLoop(argsChan <-chan []string) ([]commandFailure, error) {
	...
	dataDirectoryPath, err := common.TempDir("kobradsanity-kobrad-datadir")
	if err != nil {
	...
	for args := range argsChan {
		...
		cmd := exec.Command("kobrad", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "kobrad-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "kobrad-STDERR")
