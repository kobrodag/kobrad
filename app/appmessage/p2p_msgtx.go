...
// Outpoint defines a kobra data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new kobra transaction outpoint point with the
...
// TxIn defines a kobra transaction input.
type TxIn struct {
...
// NewTxIn returns a new kobra transaction input with the provided
...
// TxOut defines a kobra transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new kobra transaction output with the provided
...
// MsgTx implements the Message interface and represents a kobra tx message.
