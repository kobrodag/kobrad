...
import (
	...
	"github.com/kobradag/kobrad/infrastructure/os/execenv"
	"github.com/kobradag/kobrad/infrastructure/os/limits"
	"github.com/kobradag/kobrad/infrastructure/os/signal"
	"github.com/kobradag/kobrad/infrastructure/os/winservice"
...
type kobradApp struct {
	cfg *config.Config
}

// StartApp starts the kobrad app, and blocks until it finishes running
func StartApp() error {
...
func (app *kobradApp) main(startedChan chan<- struct{}) error {
	...
	if err != nil {
		log.Errorf("Unable to start kobrad: %+v", err)
		return err
	}

	defer func() {
		log.Infof("Gracefully shutting down kobrad...")
		...
		log.Infof("kobrad shutdown complete")
	}()
