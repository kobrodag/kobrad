...
import (
	...
	"github.com/kobradag/kobrad/domain/consensus"
	...
	"github.com/kobradag/kobrad/domain/consensus/utils/testutils"
...
func TestMultisig(t *testing.T) {
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		forSchnorrAndECDSA(t, func(t *testing.T, ecdsa bool) {
			...
			for i := 0; i < numKeys; i++ {
				var err error
				mnemonics[i], err = libkobrawallet.CreateMnemonic()
				if err != nil {
					t.Fatalf("CreateMnemonic: %+v", err)
				}

				publicKeys[i], err = libkobrawallet.MasterPublicKeyFromMnemonic(&consensusConfig.Params, mnemonics[i], true)
				if err != nil {
			...
			path := "m/1/2/3"
			address, err := libkobrawallet.Address(params, publicKeys, minimumSignatures, path, ecdsa)
			if err != nil {
			...
			selectedUTXOs := []*libkobrawallet.UTXO{
				{
			...
			unsignedTransaction, err := libkobrawallet.CreateUnsignedTransaction(publicKeys, minimumSignatures,
				[]*libkobrawallet.Payment{{
					Address: address,
					Amount:  10,
				}}, selectedUTXOs)
			if err != nil {
			...
			isFullySigned, err := libkobrawallet.IsTransactionFullySigned(unsignedTransaction)
			if err != nil {
			...
			_, err = libkobrawallet.ExtractTransaction(unsignedTransaction, ecdsa)
			...
			signedTxStep1, err := libkobrawallet.Sign(params, mnemonics[:1], unsignedTransaction, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			isFullySigned, err = libkobrawallet.IsTransactionFullySigned(signedTxStep1)
			if err != nil {
				t.Fatalf("IsTransactionFullySigned: %+v", err)
			}
			...
			signedTxStep2, err := libkobrawallet.Sign(params, mnemonics[1:2], signedTxStep1, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			extractedSignedTxStep2, err := libkobrawallet.ExtractTransaction(signedTxStep2, ecdsa)
			if err != nil {
				t.Fatalf("ExtractTransaction: %+v", err)
			}

			signedTxOneStep, err := libkobrawallet.Sign(params, mnemonics[:2], unsignedTransaction, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			extractedSignedTxOneStep, err := libkobrawallet.ExtractTransaction(signedTxOneStep, ecdsa)
			if err != nil {
				t.Fatalf("ExtractTransaction: %+v", err)
			}
	...
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		forSchnorrAndECDSA(t, func(t *testing.T, ecdsa bool) {
			...
			for i := 0; i < numKeys; i++ {
				...
				publicKeys[i], err = libkobrawallet.MasterPublicKeyFromMnemonic(&consensusConfig.Params, mnemonics[i], false)
				if err != nil {
					t.Fatalf("MasterPublicKeyFromMnemonic: %+v", err)
				}
			}
			...
			signedTx, err := libkobrawallet.Sign(params, mnemonics, unsignedTransaction, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			tx, err := libkobrawallet.ExtractTransaction(signedTx, ecdsa)
			if err != nil {
				t.Fatalf("ExtractTransaction: %+v", err)
			}
...
func TestMaxLeor(t *testing.T) {
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		for i := 0; i < numKeys; i++ {
			...
			publicKeys[i], err = libkobrawallet.MasterPublicKeyFromMnemonic(&cfg.Params, mnemonics[i], false)
			if err != nil {
				t.Fatalf("MasterPublicKeyFromMnemonic: %+v", err)
			}
		}
		...
		const minimumSignatures = 1
		path := "m/1/2/3"
		address, err := libkobrawallet.Address(params, publicKeys, minimumSignatures, path, false)
		if err != nil {
			t.Fatalf("Address: %+v", err)
		}
		...
		selectedUTXOsForTxWithLargeInputAmount := []*libkobrawallet.UTXO{
			{
				Outpoint: &externalapi.DomainOutpoint{
		...
		unsignedTxWithLargeInputAmount, err := libkobrawallet.CreateUnsignedTransaction(publicKeys, minimumSignatures,
			[]*libkobrawallet.Payment{{
				Address: address,
				Amount:  10,
		...
		signedTxWithLargeInputAmount, err := libkobrawallet.Sign(params, mnemonics, unsignedTxWithLargeInputAmount, false)
		if err != nil {
			t.Fatalf("Sign: %+v", err)
		}

		txWithLargeInputAmount, err := libkobrawallet.ExtractTransaction(signedTxWithLargeInputAmount, false)
		if err != nil {
			t.Fatalf("ExtractTransaction: %+v", err)
		}
		...
		selectedUTXOsForTxWithLargeInputAndOutputAmount := []*libkobrawallet.UTXO{
			{
				Outpoint: &externalapi.DomainOutpoint{
		...
		unsignedTxWithLargeInputAndOutputAmount, err := libkobrawallet.CreateUnsignedTransaction(publicKeys, minimumSignatures,
			[]*libkobrawallet.Payment{{
				Address: address,
		...
		signedTxWithLargeInputAndOutputAmount, err := libkobrawallet.Sign(params, mnemonics, unsignedTxWithLargeInputAndOutputAmount, false)
		if err != nil {
			t.Fatalf("Sign: %+v", err)
		}

		txWithLargeInputAndOutputAmount, err := libkobrawallet.ExtractTransaction(signedTxWithLargeInputAndOutputAmount, false)
		if err != nil {
			t.Fatalf("ExtractTransaction: %+v", err)
		}
