...
import (
	...
	"github.com/kobradag/kobrad/infrastructure/network/rpcclient/grpcclient"
)
...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		...
		remoteVersion := kobradMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	...
	return marshalOptions.Format(kobradMessage)
}
