...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-kobrad.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "kobrad")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of kobrad binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-kobrad.conf")
