...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of kobrad instances
	kobrad1, kobrad2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := kobrad1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In kobrad2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, kobrad2)
	}

	// Connect the two kobrads. This should trigger sync
	// between the two nodes
	connect(t, kobrad1, kobrad2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := kobrad1.rpcClient.GetVirtualSelectedParentChainFromBlock(
